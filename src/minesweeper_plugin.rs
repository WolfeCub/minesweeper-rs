...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>, game_state: Res<GameState>) {
    /* TODO: Not hardcoded */
    let mut board = Board::new(game_state.width, game_state.height);
    for _ in 0..game_state.num_bombs {
        board.add_bomb();
    }
        ...
        app.add_state(AppState::MainMenu)
            ...
            .add_system_set(
                SystemSet::on_update(AppState::MainMenu)
                    .with_system(main_menu_action_system))
            .add_system_set(
                SystemSet::on_enter(AppState::InGame)
                    .with_system(setup))
            .add_system_set(
                SystemSet::on_update(AppState::InGame)
                    .with_system(mouse_click_system.chain(|_| {}))
                    .with_system(uncover_system));
    }
}
